...
public class CountDownProgressView extends AppCompatTextView {
    ...
    public CountDownProgressView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
        @SuppressLint("CustomViewStyleable") TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.CountDownProgress);
        if (typedArray != null) {
            // 圆实心的颜色属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_circSolidColor)) {
                //有这个属性索引 获取用户设置的颜色
                circleSolidColor = typedArray.getColor(R.styleable.CountDownProgress_circSolidColor, 0);
            } else {
                //没有这个属性索引 默认是浅灰色
                circleSolidColor = typedArray.getColor(R.styleable.CountDownProgress_circSolidColor, Color.parseColor("#D3D3D3"));
            }

            // 园边框颜色属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_circFrameColor)) {
                //有这个属性索引 获取用户设置的颜色
                circleFrameColor = typedArray.getColor(R.styleable.CountDownProgress_circFrameColor, 0);
            } else {
                //没有这个属性索引 默认是深灰色
                circleFrameColor = typedArray.getColor(R.styleable.CountDownProgress_circFrameColor, Color.parseColor("#A9A9A9"));
            }
